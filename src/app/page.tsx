...
export default function JarvisChat() {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Build clean history for the API (Interference Shield)
      const cleanHistory = [...(messages || []), tempUserMsg].slice(-10).map(m => ({
      ...
      if (data?.content) {
        ...
        if (data.memoryUpdate) {
          ...
          await addDoc(collection(db, "memories"), { text: data.memoryUpdate, type: "knowledge", createdAt: serverTimestamp() });
          setTimeout(() => { setIsSyncing(false); setSyncMessage(""); }, 3000);
        }
    ...
    <div className="flex flex-col h-screen bg-[#0a0a0a] text-slate-100 font-sans relative overflow-hidden">
      ...
      <header className="px-6 py-4 border-b border-white/5 bg-black/60 backdrop-blur-xl flex justify-between items-center z-50">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => setIsMemoryOpen(true)} className={`relative flex items-center gap-2 px-3 py-1.5 rounded-lg border transition-all duration-700 text-[10px] font-mono ${isSyncing ? "bg-blue-500/20 border-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.5)]" : "border-white/10 hover:bg-white/5"}`}>
            <Brain size={14} className={isSyncing ? "text-blue-400 animate-pulse" : ""} /> RECORDS
          </button>
        </div>
      </header>
        ...
        {(messages || []).map((m, idx) => {
          ...
          return (
            <div key={m.id || `msg-${idx}`} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`group relative max-w-[85%] md:max-w-[70%] p-5 rounded-2xl transition-all duration-500 ${m.role === "user" ? "bg-blue-600/5 border border-blue-500/20" : intent === "CLINICAL" ? "bg-emerald-500/5 border border-emerald-500/20 shadow-[0_0_30px_rgba(16,185,129,0.05)]" : "bg-[#111] border border-white/5 shadow-2xl"}`}>
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {m.role === "assistant" && <span className={`text-[8px] px-1.5 py-0.5 rounded border ${intent === "CLINICAL" ? "border-emerald-500/30 text-emerald-500" : "border-blue-500/30 text-blue-500"}`}>{intent}</span>}
                </div>
                <div className={`text-sm font-light leading-relaxed whitespace-pre-wrap ${intent === "CODE" ? "font-mono bg-black/40 p-4 rounded-lg border border-white/5" : ""}`}>{m.content}</div>
                {intent === "CLINICAL" && m.role === "assistant" && (
                  <button onClick={() => handleExport(m.content)} className="mt-4 flex items-center gap-2 px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 rounded text-[9px] text-emerald-500 transition-all font-mono">
      ...
      <footer className="absolute bottom-0 left-0 w-full p-8 bg-gradient-to-t from-black via-black/90 to-transparent z-40">
        <form onSubmit={handleSendMessage} className="max-w-4xl mx-auto flex gap-4 items-center bg-[#161616]/80 border border-white/10 p-2 rounded-2xl backdrop-blur-3xl focus-within:border-blue-500/40 shadow-2xl transition-all">
          <button type="button" onClick={startListening} className={`p-4 rounded-xl transition-all ${isListening ? "text-red-500" : "text-slate-500 hover:text-blue-400"}`}><Mic size={20} className={isListening ? "animate-pulse" : ""} /></button>
          ...
          <button type="submit" disabled={isLoading || !input.trim()} className="p-4 bg-blue-600 hover:bg-blue-500 text-white rounded-xl disabled:opacity-20 shadow-lg shadow-blue-600/20 transition-all"><Send size={18} /></button>
        </form>
      </footer>
